...
class UserSignupTests(TestCase):
    def setUp(self):
        ...
        self.signup_form_fields = {
            'username': 'bar',
            'email': 'bar@email.com',
            'password1': '123456789!#Abc',
            'password2': '123456789!#Abc',
        }
    ...
    def test_signup_fails_password_confirm_notmatch(self):
        ...
        self.signup_form_fields['password2'] = self.signup_form_fields['password2'].upper()

        response = self.client.post('/accounts/signup', self.signup_form_fields)

        self.assertEqual(response.status_code, 200)
        self.assertFalse(response.context['user'].is_authenticated)
        self.assertIn("The two password fields didn’t match.", str(response.context['form'].errors))
    ...
    def test_signup_fails_username_notunique(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(response.context['user'].is_authenticated)
        self.assertIn("A user with that username already exists.", str(response.context['form'].errors))
